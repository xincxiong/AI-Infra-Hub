...
export default function AskAISidebar({ 
  ...
  const generateSummary = async () => {
    ...
    const summaryMessage: Message = {
      ...
      type: 'ai',
      content: `根据您选中的内容，这是关于「${reportTitle}」的关键信息摘要：\n\n这是今日${reportType === 'market' ? '市场动态' : reportType === 'tech' ? '技术前沿' : '产品发布'}的核心内容，反映了行业最新的发展趋势和竞争格局变化。\n\n**关键要点：**\n• 涉及领域：AI 基础设施 / 大模型推理优化\n• 事件类型：${reportType === 'market' ? '融资/合作/政策' : reportType === 'tech' ? '技术发布/论文发表' : '产品发布/功能更新'}\n• 影响程度：高 - 可能改变行业竞争格局`,
      sources: [
        { title: 'OpenAI 官方博客', url: '#' },
        { title: 'TechCrunch 报道', url: '#' },
      ],
      relatedItems: [
        { title: 'OpenAI 发布 GPT-4 Turbo 新功能', id: '1' },
        { title: 'Anthropic 宣布 Claude 3.5 升级', id: '2' },
      ],
      timestamp: new Date(),
    };
  ...
  const handleSendMessage = async () => {
    ...
    const aiMessage: Message = {
      ...
      relatedItems: [
        { title: '相关新闻：NVIDIA 发布新一代 GPU', id: '3' },
        { title: '技术解读：大模型推理优化实践', id: '4' },
      ],
      timestamp: new Date(),
    };
